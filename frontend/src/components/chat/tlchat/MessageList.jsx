...
export default function MessageList({
  ...
  // Save edit (optimistic + revert on fail)
  const saveEdit = (msg) => {
    ...
    socket.emit(
      ...
      (res) => {
        ...
        if (!res?.success) {
          ...
          // Success: backend already broadcasts
          cancelEditing();
        }
      }
    );
  ...
  return (
    <div className="flex-1 overflow-y-auto px-5 py-6 bg-gray-50/50 space-y-6">
      {messages.length === 0 ? (
        <div className="flex flex-col items-center justify-center h-full text-gray-500">
          <p className="text-lg font-medium">No messages yet</p>
          <p className="text-sm mt-2 opacity-80">Start the conversation</p>
        </div>
      ) : (
        messages.map((msg) => {
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-2 justify-end">
                      <button
                        ...
                        className="p-1.5 rounded hover:bg-gray-200/50 text-gray-600 hover:text-gray-900 transition"
                        title="Cancel"
                      >
                        <X size={16} />
                      </button>
                      <button
                        ...
                        className="p-1.5 rounded bg-blue-600 hover:bg-blue-700 text-white disabled:opacity-50 transition flex items-center gap-1"
                        title="Save"
                      >
                        {isSaving ? (
                  ...
                  >
                    {canEdit && (
                      <button
                        ...
                        className="p-1.5 rounded-full bg-white/90 shadow hover:bg-white text-gray-700 hover:text-blue-600 transition"
                        title="Edit"
                      >
                        <Pencil size={14} />
                      </button>
                    ...
                    {canDelete && (
                      <button
                        ...
                        className="p-1.5 rounded-full bg-white/90 shadow hover:bg-white text-gray-700 hover:text-red-600 transition"
                        title="Delete"
                      >
                        <Trash2 size={14} />
                      </button>
