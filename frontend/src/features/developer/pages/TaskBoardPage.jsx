...
export default function TaskBoardPage() {
    ...
    try {
      ...
      tasks.forEach((task) => {
        ...
        if (organizedTasks[status]) {
          organizedTasks[status].items.push({
            id: task._id,
            content: task.title || "",
            developer: task.assignedTo?._id || "",
            developerName: task.assignedTo?.name || "Unknown",
  ...
  const onDragEnd = async (result) => {
    ...
    // Find the task
    const task = columns[source.droppableId].items.find((item) => item.id === taskId);
    if (!task) return;
