...
export default function UserManagementPage() {
    ...
    return (
        <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 180px 180px 140px", gap: 12, marginBottom: 12 }}>
                    ...
                    <thead>
                        <tr style={{ background: "#f9fafb" }}>
                            {["Name", "Email", "Role", "Team", "Actions"].map((h) => (
                                <th key={h} style={{ textAlign: "left", padding: 12, fontSize: 13, color: "#374151" }}>{h}</th>
                            ))}
                        </tr>
                    </thead>
                    <tbody>
                        {employees.map((e) => (
                            <tr key={e._id ?? e.id} style={{ borderTop: "1px solid #eee" }}>
                                ...
                                <td style={{ padding: 12, display: "flex", gap: 8 }}>
                                    <Button variant="secondary" onClick={() => alert("Hook to View page")}>View</Button>
                                    <Button variant="secondary" onClick={() => alert("Hook to Edit page")}>Edit</Button>
                                    <Button variant="danger" onClick={() => setDeleteId(e._id ?? e.id)}>Delete</Button>
                                </td>
                            </tr>
                        ))}
                        {!loading && employees.length === 0 && (
                            <tr><td colSpan={5} style={{ padding: 16, color: "#6b7280" }}>No employees found.</td></tr>
                        )}
                    </tbody>
                </table>
            </div>
