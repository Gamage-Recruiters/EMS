...
const Register = () => {
  ...
  const validate = () => {
    ...
    if (!formData.terms) newErrors.terms = "You must agree to the terms & privacy policy";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Parse JSON if possible, otherwise capture raw text for debugging
      let data = {};
      try {
        data = await response.json();
      } catch (parseErr) {
        const text = await response.text().catch(() => "");
        console.warn("Non-JSON response from register:", text);
        if (!response.ok) {
          setErrors({ api: text || "Registration failed (no JSON message)" });
          return;
        }
      }

      if (!response.ok) {
        const message = data.message || data.error || data.msg || "Registration failed";
    ...
    <div className="min-h-screen flex">
      ...
      <div className="flex-1 flex items-center justify-center bg-slate-50">
        <div className="w-full max-w-xl px-6 py-10 md:px-10">
          <h2 className="text-3xl md:text-4xl font-semibold text-blue-600">Welcome to Gamage Recruiters</h2>
          <p className="mt-2 text-gray-500">Register your account</p>

          <form onSubmit={handleSubmit} className="mt-8 space-y-6">
            {errors.api && <p className="text-sm text-red-600 bg-red-50 border border-red-200 rounded-md px-3 py-2">{errors.api}</p>}

            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label htmlFor="firstName" className="block text-sm font-medium text-blue-700">First Name</label>
                <input id="firstName" name="firstName" type="text" className={inputClass("firstName")} placeholder="Enter first name" value={formData.firstName} onChange={handleChange} />
                {errors.firstName && <p className="mt-1 text-xs text-red-600">{errors.firstName}</p>}
              </div>

              <div>
                <label htmlFor="lastName" className="block text-sm font-medium text-blue-700">Last Name</label>
                <input id="lastName" name="lastName" type="text" className={inputClass("lastName")} placeholder="Enter last name" value={formData.lastName} onChange={handleChange} />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label htmlFor="email" className="block text-sm font-medium text-blue-700">E-mail Address</label>
                <input id="email" name="email" type="email" className={inputClass("email")} placeholder="example@gamage.com" value={formData.email} onChange={handleChange} />
                {errors.email && <p className="mt-1 text-xs text-red-600">{errors.email}</p>}
              </div>

              <div>
                <label htmlFor="phone" className="block text-sm font-medium text-blue-700">Phone Number</label>
                <input id="phone" name="phone" type="tel" className={inputClass("phone")} placeholder="07X XXXX XXX" value={formData.phone} onChange={handleChange} />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input id="password" name="password" type="password" className={inputClass("password")} placeholder="Enter password" value={formData.password} onChange={handleChange} />
                {errors.password && <p className="mt-1 text-xs text-red-600">{errors.password}</p>}
              </div>

              <div>
                <label htmlFor="confirmPassword" className="block text-sm font-medium text-blue-700">Confirm Password</label>
                <input id="confirmPassword" name="confirmPassword" type="password" className={inputClass("confirmPassword")} placeholder="Re-enter password" value={formData.confirmPassword} onChange={handleChange} />
                {errors.confirmPassword && <p className="mt-1 text-xs text-red-600">{errors.confirmPassword}</p>}
              </div>
            </div>

            <div className="flex items-start gap-2">
              <input id="terms" name="terms" type="checkbox" className="mt-1 h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500" checked={formData.terms} onChange={handleChange} />
              <label htmlFor="terms" className="text-sm text-gray-600">
                I agree to all the{" "}
                <button type="button" className="text-blue-600 underline underline-offset-2">Terms</button>,{" "}
                <button type="button" className="text-blue-600 underline underline-offset-2">Privacy Policy</button>
              </label>
            </div>
            {errors.terms && <p className="mt-1 text-xs text-red-600">{errors.terms}</p>}

            <button type="submit" disabled={isSubmitting} className={`w-full rounded-md bg-blue-600 py-2.5 text-sm font-semibold text-white shadow-sm transition-colors ${isSubmitting ? "opacity-70 cursor-not-allowed" : "hover:bg-blue-700"}`}>
